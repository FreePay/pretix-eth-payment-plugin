...
const keepIframeOpenWindowPropertyKey = '3citiesKeepIframeOpen'; // name of `window` property to track the iframe's request to keep itself open. Iff `window[keepIframeOpenWindowPropertyKey]`, requests to close the iframe or navigate away from the page should be denied where possible

(() => { // attempt to prevent navigating away from the page if 3cities requested to keep the iframe open
    window.addEventListener('beforeunload', function (e) {
        if (window[keepIframeOpenWindowPropertyKey]) {
            e.preventDefault();
            e.returnValue = 'Please don\'t leave the page while signing the transaction.';
    ...
    singleton3citiesIframeMessageEventHandler = (event) => {
        if (event.origin === tcOrigin) { // WARNING this is a crucial security check to ensure that this message has been sent from the expected 3cities iframe origin. Otherwise, any window can claim to be sending a message from 3cities
            if (event !== null && typeof event === 'object' && event.data !== null && typeof event.data === 'object') {
                ...
                else if (event.data.kind === 'TurnOnKeepIframeOpen') {
                    window[keepIframeOpenWindowPropertyKey] = true;
                }
                else if (event.data.kind === 'TurnOffKeepIframeOpen') {
                    window[keepIframeOpenWindowPropertyKey] = false;
                }
